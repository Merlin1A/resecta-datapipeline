...
# how the SEC-6 sign products leaked into locked bytes (speed review finding
...
def write_hash_lockfile(path: Path, entries: dict[str, str]) -> None:
    ...
    lines = [
        ...
        "# See CLAUDE.md §4: unexpected changes are a red flag.",
        "",
    ]
...
def iter_build_artifacts(build_dir: Path) -> list[Path]:
    ...
    - ``bundle_size.meta.json`` (D-35 build-input metadata sidecar — carries
